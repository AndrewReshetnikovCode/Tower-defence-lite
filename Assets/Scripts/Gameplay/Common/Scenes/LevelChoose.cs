...
public class LevelChoose : MonoBehaviour
{
    ...
    {
		int hitIdx = -1;
		int levelsCount = SaveLoadManager.instance.progress.openedLevels.Count;
		if (levelsCount > 0)
		{
			// Get name of last opened level from stored data
			string openedLevelName = SaveLoadManager.instance.progress.openedLevels[levelsCount - 1];
